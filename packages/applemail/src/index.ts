...
server.tool(
  ...
  "List recent emails from Apple Mail. Can filter by account, folder, and unread status.",
  {
    account: z
      ...
      .optional()
      .describe(
        "Filter by account email address (e.g., 'user@example.com')"
      ),
    folder: z
      .string()
      .optional()
...
process.on("unhandledRejection", (err) => { console.error("[applemail] Unhandled rejection:", err); process.exit(1); });
process.on("uncaughtException", (err) => { console.error("[applemail] Uncaught exception:", err); process.exit(1); });
